...
const getProducts = async (req, res) => {
  try {
    ...
    res.status(500).json({
      message: "Не удалось получить список товаров, повторите попытку",
    });
  }
};
...
const createProduct = async (req, res) => {
  ...
  if (!req.body.catalogueNumber) {
    errors.catalogueNumber = { message: "Укажите каталожный номер" };
  }

  if (!req.body.id) {
    errors.id = { message: "Укажите ID" };
  }
  ...
  try {
    ...
    res.status(500).json({
      message: "Не удалось создать товар, повторите попытку",
    });
  }
};
